...
def actor_loss_fn(actions_advantages: torch.Tensor,
                  policy_logits: torch.Tensor, char_logits) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    probs = torch.fmax(probs, 1e-10 * torch.ones_like(probs))
    ...
    if params['char_logit']:
        ...
        entropy_loss = entropy_loss_weight * (
                    -char_probs * torch.log(torch.fmax(char_probs, 1e-10 * torch.ones_like(char_probs)))).sum(
            dim=1).mean()
    else:
